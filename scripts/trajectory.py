...
class TrajectoryAnalyzer():
    ...
    def __init__(self):
        ...
        self._client=pymongo.MongoClient("localhost",62345)

        self._traj = dict()
        self._traj_robot = dict()
    ...
    def _retrieve_logs(self):
        ...
        logs = self._client.message_store.people_perception_morse.find() #.sort({'header.stamp': 1})

        for log in logs:
            if log['_meta'].has_key('perspective') and log['_meta']['perspective'] == 'robot':
                for i, uuid in enumerate(log['uuids']):
                    if uuid not in self._traj_robot:
                        t = Trajectory(uuid)
                        t.append_pose(log['people'][i],log['header']['stamp']['secs'],log['header']['stamp']['nsecs'])
                        self._traj_robot[uuid] = t
                    else:
                        t = self._traj_robot[uuid]
                        t.append_pose(log['people'][i],log['header']['stamp']['secs'],log['header']['stamp']['nsecs'])
            else:
                for i, uuid in enumerate(log['uuids']):
    ...
    def visualize_trajectories_robot(self):
        print("Local Trajectory")
        for uuid in self._traj_robot:
            if len(self._traj_robot[uuid].pose) > 1:
                self.visualize_trajectory(self._traj_robot[uuid])
                _in = raw_input("Press 'Enter' for the next trajectory.")
                self.delete_trajectory(self._traj_robot[uuid])
...
if __name__=="__main__":
    ...
    for k, v in ta._traj_robot.iteritems():
        v.calc_stats()
        print k, len(v.pose), v.length, v.max_vel
